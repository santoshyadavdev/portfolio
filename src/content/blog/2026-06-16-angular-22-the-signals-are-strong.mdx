...
The `linkedSignal` API now supports a custom `set` option, giving you more control over how linked signals behave when updated.
...
You can now debounce signals directly — great for search inputs, form validation, and other performance-sensitive reactive patterns:
...
### `injectAsync` Helper Function

The new `injectAsync` helper makes lazy-loading services straightforward — load a dependency only when it's actually needed:

```ts
export class InjectAsyncDemo {
  // Load on demand — AnalyticsService is NOT in the initial bundle.
