...
When you put `formControlName`, `formControl`, or `ngModel` on a native `<input>`, Angular automatically attaches a ControlValueAccessor (CVA) , a bridge that syncs the input's DOM value with the reactive form model.

**The problem:** When you use the new signal forms `[formField]` directive on an input that's also inside a `[formGroup]`, both systems try to control the input.

`ngNoCva` solves this , it's a simple attribute that tells Angular: *"Don't attach a CVA here, I'm using signal forms instead."*

**When you need it:**
- When using `[formField]` on an input inside a `[formGroup]` or with `formControlName`/`ngModel`
- For the `SignalFormControl` compat scenario (mixing old reactive forms with new signal forms)
...
- If you're using `[formField]` without any reactive forms context , no CVA is attached, so no conflict
- If you're using `[formField]` on a custom `FormValueControl` component , those don't use CVA at all
...
```ts
  ...
  // ════════════════════════════════════════════════════════════
  //  Custom `set` option , write-back to source signal
